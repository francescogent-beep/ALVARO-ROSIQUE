    ...
    <>
        ...
        <Container className="relative z-10 pt-20 pb-24 lg:pt-32 lg:pb-40 w-full">
          <div className="animate-in fade-in slide-in-from-bottom duration-1000 w-full flex flex-col items-center lg:items-start">
            <div className="text-center lg:text-left max-w-5xl lg:mx-0 w-full">
              ...
              <H1 className="mb-8 md:mb-12 lg:mb-16 !text-brand-950 leading-[1.1] lg:leading-[1] tracking-tighter drop-shadow-sm py-[10px] text-center lg:text-left">
                Tu salud en<br className="hidden lg:block" />  <span className="text-blue-600 italic">buenas manos</span>.
              </H1>
              <Text className="mb-12 md:mb-20 max-w-3xl mx-auto lg:mx-0 !text-brand-950 text-lg md:text-2xl lg:text-3xl font-medium leading-relaxed tracking-tight drop-shadow-sm text-center lg:text-left opacity-90">
      ...
      <Section className="bg-brand-950 text-white relative overflow-hidden">
        ...
        <div className="absolute top-0 left-0 w-full h-full opacity-5 pointer-events-none">
          ...
          <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-blue-500 rounded-full blur-[150px]"></div>
        </div>
        ...
        <Container className="relative z-10">
          <div className="text-center max-w-5xl mx-auto mb-20 md:mb-32">
            ...
            <h2 className="text-4xl md:text-6xl lg:text-7xl font-bold mb-8 md:mb-12 tracking-tighter leading-tight">¿Qué te duele hoy?</h2>
            <p className="text-slate-400 text-lg md:text-xl lg:text-2xl font-light avenir-minimal max-w-3xl mx-auto leading-relaxed">No solo tratamos el dolor, buscamos por qué aparece. Te ayudamos a entender tu cuerpo para que te sientas bien de verdad.</p>
          </div>
          <Grid cols={3}>
            {SITE_DATA.painPoints.map((p, i) => (
              <div key={i} className="p-10 md:p-16 border border-white/5 rounded-[3rem] md:rounded-[4rem] bg-white/[0.02] hover:bg-white/[0.05] transition-all duration-1000 group flex flex-col h-full relative overflow-hidden">
                <div className="absolute top-0 right-0 p-10 opacity-5 group-hover:opacity-10 transition-opacity">
                  <Zap size={80} />
                </div>
                <div className="mb-8 md:mb-12 p-5 md:p-6 bg-blue-900/40 rounded-2xl md:rounded-[2rem] w-fit border border-blue-500/20">
                  <Zap className="text-blue-400 w-6 h-6 md:w-8 md:h-8" />
                </div>
                <h3 className="text-2xl md:text-4xl font-semibold mb-6 md:mb-8 tracking-tighter">{p.title}</h3>
                <p className="text-slate-400 font-light leading-relaxed mb-10 md:mb-16 flex-grow text-base md:text-lg lg:text-xl avenir-minimal">{p.desc}</p>
                <div className="pt-8 md:pt-10 border-t border-white/10 flex items-center justify-between">
                  <span className="text-blue-400 font-black uppercase text-[10px] md:text-[11px] tracking-luxury">
                    Objetivo: {p.solution}
                  </span>
                  <ArrowRight size={16} className="text-white/20 group-hover:text-blue-400 group-hover:translate-x-2 transition-all" />
                </div>
              </div>
            ))}
          </Grid>
          <div className="mt-20 md:mt-32 flex flex-col items-center">
            <div className="h-20 w-[1px] bg-gradient-to-b from-transparent to-blue-500 mb-10"></div>
          ...
          <div className="grid lg:grid-cols-2 gap-16 md:gap-32 lg:gap-40 items-center">
            <div className="relative group order-2 lg:order-1">
               ...
               <div className="absolute -bottom-12 -right-12 md:-bottom-24 md:-right-24 bg-brand-950 text-white p-10 md:p-20 rounded-[2rem] md:rounded-[4rem] hidden xl:block shadow-3xl">
                  <ShieldCheck className="mb-4 md:mb-8 text-blue-400 w-10 h-10 md:w-14 md:h-14" />
            ...
            <div className="lg:pl-12 order-1 lg:order-2">
              ...
              <h2 className="text-4xl md:text-5xl lg:text-6xl font-semibold tracking-tighter leading-tight text-brand-950 mb-8 md:mb-12">Cómo trabajamos.</h2>
              <Text className="mb-12 md:mb-20 text-lg md:text-xl lg:text-2xl">
              ...
              <div className="space-y-10 md:space-y-16">
                {SITE_DATA.methodology.map((m, i) => (
                  <div key={i} className="flex space-x-6 md:space-x-12 items-start">
                    <span className="text-4xl md:text-6xl font-black text-brand-50 leading-none">{m.step}</span>
                    <div>
                      <h4 className="text-xl md:text-2xl lg:text-3xl font-bold text-brand-950 mb-2 md:mb-4 tracking-tight">{m.title}</h4>
                      <p className="text-slate-500 font-light leading-relaxed text-base md:text-lg lg:text-xl avenir-minimal">{m.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-8 md:space-y-12">
              {["Agenda Preferente", "Tratamiento Personalizado", "Evaluación Postural"].map((item, i) => (
                <div key={i} className="flex items-center space-x-6 md:space-x-8 group">
                  <div className="w-3 h-3 md:w-4 md:h-4 rounded-full border-2 md:border-4 border-blue-600 group-hover:bg-blue-600 transition-all duration-500"></div>
              ...
              >
                <div className="aspect-[16/9] overflow-hidden relative">
                  ...
                  <div className="absolute top-6 left-6">
                    <span className="px-4 py-2 bg-white/90 backdrop-blur-md rounded-full text-[10px] font-black uppercase tracking-luxury text-blue-600 shadow-sm">
                      {post.category}
                    </span>
                  </div>
                </div>
                <div className="p-8 md:p-12">
                  ...
                  <h3 className="text-2xl md:text-3xl font-bold text-brand-950 mb-6 group-hover:text-blue-600 transition-colors leading-tight">
                    {post.title}
                  </h3>
                  <p className="text-slate-500 font-light leading-relaxed text-base md:text-lg avenir-minimal line-clamp-2 mb-8">
                    {post.excerpt}
                  </p>
                  <div className="flex items-center text-blue-600 font-black uppercase tracking-luxury text-[10px]">
        ...
        <Container>
          <div className="grid lg:grid-cols-2 gap-16 md:gap-32 lg:gap-40 items-start">
            <div>
              ...
              <h2 className="text-4xl md:text-5xl lg:text-6xl font-semibold tracking-tighter leading-tight text-brand-950 mb-8 md:mb-12">Cuida de ti.</h2>
              <Text className="mb-12 md:mb-20 text-lg md:text-xl lg:text-2xl avenir-minimal">La clínica de Álvaro Rosique está en pleno centro de Cartagena. Un espacio tranquilo donde lo más importante eres tú.</Text>
              
              <div className="space-y-6 md:space-y-10 mb-12 md:mb-20">
                <div className="flex items-center space-x-6 md:space-x-10 group">
                  <div className="w-12 h-12 md:w-16 md:h-16 bg-white rounded-2xl md:rounded-[1.5rem] flex items-center justify-center border border-brand-100 shadow-sm group-hover:border-blue-600 transition-colors duration-500">
                    <Phone className="text-blue-600 w-5 h-5 md:w-7 md:h-7" />
                  </div>
                  <a href={`tel:${SITE_DATA.contact.phone}`} className="text-xl md:text-3xl font-bold text-brand-950 hover:text-blue-600 transition-colors">{SITE_DATA.contact.phone}</a>
                </div>

                <div className="flex items-center space-x-6 md:space-x-10 group">
                  <div className="w-12 h-12 md:w-16 md:h-16 bg-white rounded-2xl md:rounded-[1.5rem] flex items-center justify-center border border-brand-100 shadow-sm group-hover:border-blue-600 transition-colors duration-500">
                    <Mail className="text-blue-600 w-5 h-5 md:w-7 md:h-7" />
                  </div>
                  <a href={`mailto:${SITE_DATA.contact.email}`} className="text-xl md:text-3xl font-bold text-brand-950 hover:text-blue-600 transition-colors tracking-tight">{SITE_DATA.contact.email}</a>
                </div>

                <div className="flex items-center space-x-6 md:space-x-10 group">
                  <div className="w-12 h-12 md:w-16 md:h-16 bg-white rounded-2xl md:rounded-[1.5rem] flex items-center justify-center border border-brand-100 shadow-sm group-hover:border-blue-600 transition-colors duration-500">
                    <MapPin className="text-blue-600 w-5 h-5 md:w-7 md:h-7" />
                  </div>
                  <div className="text-xl md:text-3xl font-bold text-brand-950 uppercase tracking-tighter leading-none">
                    ...
                    <span className="text-blue-600/70 text-sm md:text-lg">{SITE_DATA.contact.postalCode} {SITE_DATA.contact.city}</span>
                  </div>
                </div>
              </div>
