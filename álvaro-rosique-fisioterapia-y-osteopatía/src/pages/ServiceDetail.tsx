...
const ServiceDetail: React.FC = () => {
  ...
  return (
    <>
      ...
      <Section className="pb-0 pt-12 md:pt-20 lg:pt-24">
        <Container>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 md:gap-24 lg:gap-32 items-center">
            <div className="order-2 lg:order-1 animate-in fade-in slide-in-from-left duration-1000">
              ...
              <H1 className="mb-8 md:mb-10 text-4xl md:text-5xl lg:text-6xl tracking-tighter leading-none">{service.title}</H1>
              <Text className="mb-10 md:mb-14 text-xl md:text-2xl lg:text-2xl font-medium text-brand-950 leading-snug tracking-tight">
                {service.description}
              </Text>
              ...
              <div className="space-y-6 md:space-y-8 mb-10 md:mb-16">
                {service.benefits.map((b, i) => (
                  <div key={i} className="flex items-start space-x-6 group">
                    <CheckCircle2 className="text-emerald-600 shrink-0 mt-1 group-hover:scale-110 transition-transform" size={20} />
              ...
              <div className="flex flex-col sm:flex-row gap-6 md:gap-8">
                <Button href={SITE_DATA.contact.whatsapp} variant="primary" className="h-16 md:h-20 px-12 text-sm shadow-2xl">Pedir mi cita</Button>
                <Button href={`tel:${SITE_DATA.contact.phone}`} variant="outline" className="h-16 md:h-20 px-12 text-sm border-brand-950/10 text-brand-950 hover:bg-brand-50">Preguntar dudas</Button>
              </div>
            </div>
            <div className="relative order-1 lg:order-2 animate-in fade-in slide-in-from-right duration-1000">
              <div className="rounded-[3rem] md:rounded-[6rem] overflow-hidden shadow-3xl w-full aspect-[4/5] relative border border-slate-100">
                <img 
      ...
      <Section className="py-20 md:py-32">
        <Container>
          <div className="bg-slate-50 rounded-[3rem] md:rounded-[6rem] p-10 md:p-20 lg:p-32 border border-slate-100 text-center relative overflow-hidden">
            <div className="relative z-10">
              <Badge className="bg-white border-slate-200">Cómo lo hacemos</Badge>
              <H2 className="mb-8 md:mb-12 tracking-tighter leading-none">Nuestro plan para ti.</H2>
              <Text className="mb-0 max-w-5xl mx-auto text-xl md:text-2xl lg:text-3xl leading-relaxed avenir-minimal text-slate-600">{service.process}</Text>
            </div>
                ...
                {service.faqs.map((f, i) => (
                  <Card key={i} className="bg-slate-50/50 border-none p-8 md:p-12 hover:bg-slate-50 transition-colors">
                    <div className="flex items-start space-x-4 md:space-x-6">
                      <HelpCircle className="text-emerald-600 shrink-0 mt-1" size={24} />
                      <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {SITE_DATA.services.filter(s => s.slug !== slug).map(s => (
              <Link 
                key={s.slug} 
                to={`/servicios/${s.slug}`} 
                className="bg-white p-6 md:p-8 rounded-2xl md:rounded-3xl border border-slate-100 hover:border-emerald-600 hover:text-emerald-600 transition-all shadow-sm font-black text-brand-950 text-center uppercase text-[10px] md:text-[11px] tracking-widest"
              >
                {s.title}
              </Link>
            ))}
