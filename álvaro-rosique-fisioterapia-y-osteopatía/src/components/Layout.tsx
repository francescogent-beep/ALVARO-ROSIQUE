...
export const FaqPreviewSection = () => {
  ...
  return (
    <Section className="bg-slate-50 border-t border-slate-100 py-12 md:py-20 lg:py-24">
      <Container>
        <div className="flex flex-col md:flex-row justify-between items-end mb-10 md:mb-16 gap-6">
          <div className="max-w-2xl">
            <div className="flex items-center space-x-3 text-emerald-600 font-black uppercase tracking-luxury text-[9px] md:text-[10px] mb-4">
              <HelpCircle size={14} />
              <span>Dudas comunes</span>
            </div>
            <H2 className="mb-0 tracking-tighter leading-tight">Preguntas Frecuentes.</H2>
          </div>
          <Link 
            to="/preguntas-frecuentes" 
            className="group flex items-center space-x-3 text-[10px] md:text-[11px] font-black uppercase tracking-luxury text-brand-950 hover:text-emerald-600 transition-colors"
          >
        ...
        <Grid cols={3} className="gap-8 md:gap-10 lg:gap-12">
          {previewFaqs.map((f, i) => (
            <Card key={i} className="p-8 md:p-10 border-none shadow-sm hover:shadow-xl transition-all duration-700 bg-white rounded-[2rem] md:rounded-[3rem]">
              <h4 className="text-lg md:text-xl font-bold text-brand-950 mb-4 tracking-tight leading-snug">{f.q}</h4>
...
const Layout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
        ...
        <Container>
          <div className="flex justify-between items-center h-20 lg:h-28">
            ...
            <nav className="hidden lg:flex lg:space-x-8 xl:space-x-12">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`text-[12px] xl:text-[14px] font-extrabold uppercase tracking-luxury transition-all hover:text-brand-900 relative group ${
            ...
            <div className="hidden lg:flex items-center space-x-6">
              <a href={`tel:${SITE_DATA.contact.phone}`} className="text-[14px] xl:text-[15px] font-bold text-brand-950 tracking-widest">
                {SITE_DATA.contact.phone}
              </a>
              <Link
                to="/contacto"
                className="px-6 py-3.5 bg-brand-950 text-white rounded-full text-[12px] font-bold uppercase tracking-luxury hover:bg-emerald-800 transition-all"
              >
                Cita
              </Link>
            </div>
        ...
        <div className="flex flex-col h-full pt-32 pb-12 px-8">
          <nav className="flex flex-col space-y-8 flex-grow">
            {navLinks.map((link) => (
              ...
              >
                {link.name}
                {location.pathname === link.path && <ChevronRight className="text-emerald-500" size={32} />}
              </Link>
            ))}
          </nav>
      ...
      {/* Footer */}
      <footer className="bg-brand-950 border-t border-white/5 pt-16 md:pt-24 pb-40 md:pb-16 lg:pb-20 text-white">
        <Container>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-12 md:gap-20 mb-16 md:mb-20">
            <div>
              ...
              <p className="text-slate-400 font-light leading-relaxed mb-8 text-base md:text-lg xl:text-lg avenir-minimal">{SITE_DATA.description}</p>
              <div className="flex space-x-8 text-[9px] md:text-[10px] font-extrabold uppercase tracking-luxury">
                {SITE_DATA.socials.map((s) => (
                  <a key={s.platform} href={s.url} className="text-emerald-400 hover:text-white transition-colors">
                    {s.platform}
                  </a>
                ))}
              </div>
            ...
            <div className="xl:pl-16">
              <h4 className="text-[9px] md:text-[10px] font-extrabold uppercase tracking-luxury text-emerald-400 mb-8">Navegación</h4>
              <ul className="space-y-4 md:space-y-5 text-[9px] md:text-[10px] font-bold uppercase tracking-luxury">
                {navLinks.map((link) => (
                  <li key={link.path}>
            ...
            <div>
              <h4 className="text-[9px] md:text-[10px] font-extrabold uppercase tracking-luxury text-emerald-400 mb-8">El Centro</h4>
              <ul className="space-y-6 md:space-y-6 text-sm">
                <li className="flex items-start space-x-4 md:space-x-5">
                  <MapPin className="text-emerald-400 shrink-0 w-4 h-4 md:w-[16px] md:h-[16px]" />
                  <span className="text-slate-400 font-bold uppercase tracking-widest text-[9px] md:text-[10px]">{SITE_DATA.contact.address}, {SITE_DATA.contact.city}</span>
                </li>
                <li className="flex items-center space-x-4 md:space-x-5">
                  <Phone className="text-emerald-400 shrink-0 w-4 h-4 md:w-[16px] md:h-[16px]" />
                  <a href={`tel:${SITE_DATA.contact.phone}`} className="text-white font-bold text-sm md:text-base tracking-widest hover:text-emerald-400 transition-colors">{SITE_DATA.contact.phone}</a>
                </li>
              </ul>
            </div>

            <div>
              <h4 className="text-[9px] md:text-[10px] font-extrabold uppercase tracking-luxury text-emerald-400 mb-8">Horarios</h4>
              <div className="space-y-3 md:space-y-3">
                {SITE_DATA.openingHours.map(o => (
                   <div key={o.day} className="flex justify-between text-[9px] md:text-[10px] font-bold uppercase tracking-widest border-b border-white/5 pb-2 md:pb-3">
          ...
          <div className="pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center text-[9px] md:text-[10px] text-white/20 font-extrabold uppercase tracking-luxury">
