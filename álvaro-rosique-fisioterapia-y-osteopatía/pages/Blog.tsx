          ...
          <Grid cols={2}>
            {SITE_DATA.blog.map(post => (
              <Card key={post.id} className="p-0 overflow-hidden flex flex-col group cursor-pointer">
                ...
                <div className="p-8">
                  <div className="flex items-center space-x-4 text-xs font-bold text-teal-600 uppercase tracking-widest mb-4">
                    <span className="flex items-center"><Calendar size={14} className="mr-1" /> {post.date}</span>
                    <span className="flex items-center"><Tag size={14} className="mr-1" /> {post.category}</span>
                  </div>
                  <h3 className="text-2xl font-bold mb-4 group-hover:text-teal-600 transition">{post.title}</h3>
                  <p className="text-slate-600 mb-6">{post.excerpt}</p>
                  <span className="text-teal-600 font-bold underline decoration-teal-200 underline-offset-4">Seguir leyendo</span>
                </div>
              </Card>
            ))}
          </Grid>
